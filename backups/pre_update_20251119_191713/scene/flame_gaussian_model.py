...
class FlameGaussianModel(GaussianModel):
    def __init__(self, sh_degree : int, disable_flame_static_offset=False, not_finetune_flame_params=False, n_shape=300, n_expr=100, is_gumbel=False):
        ...
        # Gumbel network for learnable point selection (Phase B)
        self.is_gumbel = is_gumbel
        self.gumbel_net = None
        if is_gumbel:
            from .gumbel_network import FlameGumbelNetwork
            self.gumbel_net = FlameGumbelNetwork(W=32, use_binding_hint=True).cuda()
    ...
    def capture(self):
        """Capture model state for checkpointing, including Gumbel network if enabled."""
        base_state = super().capture()
        
        if self.is_gumbel and self.gumbel_net is not None:
            gumbel_state = self.gumbel_net.state_dict()
            return base_state + (gumbel_state,)
        else:
            return base_state + (None,)
    
    def restore(self, model_args, training_args):
        """Restore model state from checkpoint, including Gumbel network if enabled."""
        if self.is_gumbel:
            # Extract Gumbel state from model_args
            *base_args, gumbel_state = model_args
            base_state = tuple(base_args)
            
            # Restore base model state
            super().restore(base_state, training_args)
            
            # Restore Gumbel network state
            if gumbel_state is not None and self.gumbel_net is not None:
                self.gumbel_net.load_state_dict(gumbel_state)
        else:
            # No Gumbel network, restore normally
    ...
    def training_setup(self, training_args):
        ...
        # Gumbel network parameters (if enabled)
        if self.is_gumbel and self.gumbel_net is not None:
            gumbel_lr = getattr(training_args, 'gumbel_lr', 1e-4)  # Default learning rate
            param_gumbel = {'params': self.gumbel_net.parameters(), 'lr': gumbel_lr, "name": "gumbel"}
            self.optimizer.add_param_group(param_gumbel)
